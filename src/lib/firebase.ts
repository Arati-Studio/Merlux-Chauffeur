...
const firestoreDatabaseId = import.meta.env.VITE_FIREBASE_FIRESTORE_DATABASE_ID || (config as any).firestoreDatabaseId;

// Initialize Firebase SDK
const app = initializeApp(firebaseConfig);
export const auth = getAuth(app);

// Initialize Firestore with Persistent Local Cache & Multi-Tab Web Integration
...
async function testConnection() {
  try {
    ...
    if(error instanceof Error && error.message.includes('the client is offline')) {
      console.warn("Firestore connection check client offline. Accessing local cached configurations if available.");
    }
  }
}
testConnection();
